...
export function InvestigatorDashboard({ userRole = "investigator" }: InvestigatorDashboardProps) {
  ...
  const openCases = mockCases.length;
  const highRiskCases = mockCases.filter((c) => c.risk.level === "HIGH" || c.risk.level === "CRITICAL").length;
  const pendingReviews = mockCases.filter((c) => c.recommendation === "VERIFY").length;
  const escalatedCases = mockCases.filter((c) => c.recommendation === "ESCALATE").length;

  const riskDistribution = {
    HIGH: mockCases.filter((c) => c.risk.level === "HIGH").length,
    MEDIUM: mockCases.filter((c) => c.risk.level === "MEDIUM").length,
    LOW: mockCases.filter((c) => c.risk.level === "LOW").length,
  };
